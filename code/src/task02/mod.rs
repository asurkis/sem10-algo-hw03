...
#[derive(Debug)]
struct Node {
    // Запрос на обновление, покрывающий всё дерево.
    // Голова списка --- произведение на поддереве.
    // Поддерживаем инвариант: при выходе из поддерева произведение на нём актуально.
    // Хвост списка нужен только чтобы передать его ниже по дереву.
    pending: Rc<List>,
    len: usize,
